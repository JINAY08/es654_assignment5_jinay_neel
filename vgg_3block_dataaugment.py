...
def run_test_harness():
    # define model
    model = define_model()
    logs="logs/fit/vgg_3blockdataaug/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    # prepare iterators
    train_it = train_datagen.flow_from_directory('dataset_jackel_vs_nilgai/train/',
    class_mode='binary', batch_size=40, target_size=(200, 200))
    test_it = test_datagen.flow_from_directory('dataset_jackel_vs_nilgai/test/',
    class_mode='binary', batch_size=40, target_size=(200, 200))
    # fit model
    ...
    train_accuracy = history.history['accuracy'][-1]*100.0
