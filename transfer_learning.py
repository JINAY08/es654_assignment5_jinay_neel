...
def run_test_harness():
	# define model
	model = define_model()
	logs="logs/fit/vgg_3block/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
	...
	train_it = datagen.flow_from_directory('dataset_jackel_vs_nilgai/train/',
		class_mode='binary', batch_size=40, target_size=(220, 220))
	test_it = datagen.flow_from_directory('dataset_jackel_vs_nilgai/test/',
		class_mode='binary', batch_size=40, target_size=(220, 220))
	# fit model
	start_time = time.time()
	history = model.fit(train_it, steps_per_epoch=len(train_it),
		validation_data=test_it, validation_steps=len(test_it), epochs=10, verbose=1, callbacks=[tensorboard_callback])
	end_time = time.time()
